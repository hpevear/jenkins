node{
  ...
  stage('Copy Jar to S3'){
	  dir(''){
	    ...
	    sh ''' tar cvf jenkins.tar appspec.yml scripts/*.sh target/*.jar '''
            withAWS(region:'us-east-1',credentials:'aws-key') {
                def identity=awsIdentity();//Log AWS credentials
                // Upload files from working directory 'dist' in your project workspace
                 s3Upload(file:'jenkins.tar', bucket:'myjenkins-artifacts-deploy', metadatas:['x-amz-meta-application-name:MyJenkins', 'x-amz-meta-deploymentgroup-name:MyJenkins'])
            }
        };
  }
